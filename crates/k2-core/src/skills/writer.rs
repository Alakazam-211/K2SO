...
/// from source files, gives absolute paths to those sources, and asks
/// it to confirm before writing — these files affect every future wake.
///
/// `agent_name` is optional: per-agent SKILLs pass `Some(name)` so the
/// footer cites that specific agent's AGENT.md path; the workspace
...
/// routes (moved out of `k2_core::agents::*` in 0.37.0).
...
pub fn write_agent_skill_file(project_path: &str, agent_name: &str, agent_type: &str) {
    ...
    let (mut skill_content, skill_type_tag, skill_version) = match agent_type {
    ...
    // Append the "how to update" footer with absolute paths to PROJECT.md
    // and this agent's AGENT.md so the AI knows where the sources live
    // and that editing the SKILL itself will be overwritten.
