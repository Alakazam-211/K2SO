...
use k2so_core::db::schema::{AgentHeartbeat, HeartbeatFire, WorkspaceSession};
...
async fn use_workspace_session_routes_through_deliver_live_and_leaves_heartbeat_fields_untouched() {
    ...
    let result = k2so_daemon::heartbeat_launch::smart_launch(&project_path, "uws-hb");
...
async fn flag_off_uses_legacy_heartbeat_cascade_unchanged() {
    ...
    let result = k2so_daemon::heartbeat_launch::smart_launch(&project_path, "legacy-hb");
...
async fn chat_tab_and_heartbeat_register_under_separate_canonical_keys() {
    ...
    let chat_terminal = k2so_daemon::wake_headless::spawn_wake_headless(
        "manager",
        &project_path,
        "chat tab opens",
        None,
    ...
    let result = k2so_daemon::heartbeat_launch::smart_launch(&project_path, "lane-hb");
    assert_eq!(
        result.get("success").and_then(|v| v.as_bool()),
        Some(true),
