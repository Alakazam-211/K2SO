...
pub fn recompute_and_broadcast_active() {
    ...
    match k2so_core::projects_ops::compute_active_project_ids(now_ms, window) {
        Ok(ids) => {
...
async fn reconcile_pass(armed: &mut HashMap<String, ArmState>, grace_dur: Duration) {
    ...
    for (chat, forced) in to_fire {
        ...
        let recheck = tokio::task::spawn_blocking(move || {
            let window = k2so_core::app_settings::load().active_window_hours;
            let now = unix_now_ms();
            let rows = k2so_core::projects_ops::active_project_rows().unwrap_or_default();
            let active = k2so_core::active::is_in_active_set(now, &rows, window, &pid);
...
fn collect_snapshot(now_ms: i64, window: u32) -> Snapshot {
    let rows = k2so_core::projects_ops::active_project_rows().unwrap_or_default();
    let active_ids: HashSet<String> = k2so_core::active::active_project_ids(now_ms, &rows, window)
        .into_iter()
        .collect();
    ...
    for (agent_name, session) in v2_session_map::snapshot() {
        ...
        let project_id =
            match k2so_core::workspace::agent_identity::resolve_project_id(&conn, &cwd) {
                Some(id) => id,
                None => continue,
            };
