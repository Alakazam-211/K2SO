...
/// `TerminalManager` (`k2so_core::terminal::shared()`). No `AppHandle`
...
impl TerminalProvider for DaemonTerminalProvider {
    fn get_grid(&self, terminal_id: &str) -> Result<GridUpdate, String> {
        let manager = k2so_core::terminal::shared();
...
pub fn register(event_tx: broadcast::Sender<WireEvent>) {
    ...
    k2so_core::companion::terminal_bridge::set_provider(Box::new(DaemonTerminalProvider));
    k2so_core::companion::event_sink::set_sink(Box::new(DaemonCompanionEventSink {
        tx: event_tx.clone(),
    }));
    k2so_core::companion::app_event_source::set_source(Box::new(DaemonAppEventSource {
        tx: event_tx,
        handle,
    }));
    log_debug!(
...
pub fn maybe_autostart() {
    std::thread::spawn(|| {
        ...
        let snap = k2so_core::companion::settings_bridge::read_settings();
        if !snap.auto_start {
            return;
        }
        ...
        if !k2so_core::companion::auth::has_password() {
            log_debug!(
        ...
        for (i, delay) in delays.iter().enumerate() {
            ...
            match k2so_core::companion::start_companion() {
                Ok(url) => {
