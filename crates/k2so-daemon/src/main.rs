...
/// same as missing.
#[allow(dead_code)] // exposed for tests + future-handler use; today's
                    // dispatch reads `project_path` / `project` inline
                    // via `need_project` in `cli.rs`.
...
pub(crate) fn handle_agents_triage(project_path: &str) -> String {
...
pub(crate) fn handle_archive_orphans(body: &[u8]) -> cli::CliResponse {
...
pub(crate) fn dispatch_unit6_post(path: &str, body: &[u8]) -> cli::CliResponse {
    match path {
        // Filesystem
