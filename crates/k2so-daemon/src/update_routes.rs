//! P3 — daemon-side remote self-UPDATE for the STANDALONE `k2so-daemon`
...
pub struct Artifact {
    /// Direct download URL for the `k2so-daemon-<os>-<arch>` binary.
    pub url: String,
...
fn run_download_stage(job_id: &str, artifact: &Artifact) {
    ...
    let staged = dir.join("k2so-daemon");
...
pub fn handle_apply(
    ...
    tokio::spawn(async move {
        ...
        k2so_core::log_debug!("[daemon] P3 update/apply — triggering graceful shutdown for swap");
        let _ = tx.send(());
    });
...
# Generated by k2so-daemon update/apply. e2e-smoke-test-pending.
set -u
DAEMON_PID={pid}
STAGED="{staged}"
RUNNING="{running}"
...
mod tests {
    ...
    const SAMPLE_MANIFEST: &str = r#"{
      ...
      "artifacts": {
        "macos-aarch64": {
          "url": "https://releases.k2.dev/k2so-daemon-macos-aarch64",
          "sig": "https://releases.k2.dev/k2so-daemon-macos-aarch64.sig",
          "sha256": "abc123"
        },
        "linux-x86_64": {
          "url": "https://releases.k2.dev/k2so-daemon-linux-x86_64",
          "sig": "https://releases.k2.dev/k2so-daemon-linux-x86_64.sig",
          "sha256": "def456"
        }
      }
    }"#;
    ...
    fn job_lifecycle_create_get_update() {
        ...
        update_job(&id, |j| {
            j.phase = Phase::Staged;
            j.staged_path = Some(PathBuf::from("/tmp/k2so-daemon"));
        });
        let j = get_job(&id).unwrap();
        assert_eq!(j.phase, Phase::Staged);
        assert_eq!(j.staged_path, Some(PathBuf::from("/tmp/k2so-daemon")));
    ...
    fn prepare_apply_rejects_missing_staged_file() {
        ...
        update_job(&id, |j| {
            j.phase = Phase::Staged;
            j.staged_path = Some(PathBuf::from("/no/such/staged/k2so-daemon"));
        });
    ...
    fn swap_helper_script_has_expected_shape() {
        let plan = ApplyPlan {
            ...
            staged_path: PathBuf::from("/home/u/.k2so/update/abc/k2so-daemon"),
            running_path: PathBuf::from("/usr/local/bin/k2so-daemon"),
        ...
        assert!(s.contains("/usr/local/bin/k2so-daemon"), "running path");
    }
}
