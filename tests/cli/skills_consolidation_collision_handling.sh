#!/usr/bin/env bash
# 0.39.0g Phase 2.5b — collision priority: when the same name exists
# as BOTH an instance (`.k2so/agents/frontend-eng/`) AND a template
# (`.k2so/agent-templates/frontend-eng/`), the instance wins and the
# template gets a `-template01` suffix.
...
mkdir -p "$WS/.k2so/agents/frontend-eng" "$WS/.k2so/agent-templates/frontend-eng"
cat >"$WS/.k2so/agents/frontend-eng/SKILL.md" <<'EOF'
INSTANCE-WINS
EOF
cat >"$WS/.k2so/agent-templates/frontend-eng/AGENT.md" <<'EOF'
TEMPLATE-GETS-SUFFIX
EOF
...
echo "OK: collision resolved — instance wins, template gets -template01 suffix"
